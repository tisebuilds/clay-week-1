...
function CoinIcon() {
  return <Coins className="size-[13px] text-[#15803d]" strokeWidth={2} />;
}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isHovered ? "bg-[#EBF5FF]/60" : "hover:bg-neutral-50/80"
      )}
    >
      <Icon
        className={cn(
...
function SegmentedControl({
  ...
  const totalGaps = (tabCount - 1) * TAB_GAP_PX;
  const tabWidth = `calc((100% - ${TAB_PADDING_PX * 2}px - ${totalGaps}px) / ${tabCount})`;
  const indicatorOffset = `calc(${TAB_PADDING_PX}px + ${activeIndex} * (${tabWidth} + ${TAB_GAP_PX}px))`;

  return (
    <div className="relative rounded-lg bg-neutral-100 p-1">
      <div
        aria-hidden
        className="pointer-events-none absolute top-1 bottom-1 rounded-md bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08)] transition-[left] duration-200 ease-out"
        style={{
          left: indicatorOffset,
          width: tabWidth,
        }}
      />
      <div className="relative grid grid-cols-4 gap-[4.5px]">
        {categories.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <button
              ...
              onClick={() => onChange(id)}
              className={cn(
                "relative z-10 flex flex-col items-center gap-1.5 rounded-md px-1 py-2.5 transition-colors",
                isActive ? "text-[#2563EB]" : "text-neutral-500 hover:text-neutral-700"
              )}
            >
...
export function ActionsPanelV1() {
  ...
  return (
    <aside className="flex h-full w-[360px] shrink-0 flex-col overflow-hidden border-l border-neutral-200 bg-white shadow-[-8px_0_32px_rgba(0,0,0,0.06)]">
